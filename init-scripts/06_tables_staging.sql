...
-- ORDER   : Run AFTER 05_schemas.sql
-- GRAIN   : One section per table, in FK-safe creation order:
...
-- AUDIT   : All tables include created_at and updated_at for tracking
-- =============================================================================
...
-- Source: MinIO s3://raw/jeepney/routes/date=YYYY-MM-DD/routes.parquet
...
COMMENT ON COLUMN staging.stg_routes.peak_frequency_min IS
  'Headway in minutes between consecutive trips during AM/PM peak hours.';
...
COMMENT ON COLUMN staging.stg_routes.is_active IS
  'FALSE if route is suspended or rerouted under the PUV Modernization Program.';
COMMENT ON COLUMN staging.stg_routes.updated_at IS
  'Timestamp of last record update. Used for incremental load detection.';


-- =============================================================================
-- 2. staging.stg_operators
-- Grain : One row per franchise holder or cooperative
-- Source: MinIO s3://raw/jeepney/operators/date=YYYY-MM-DD/operators.parquet
...
COMMENT ON COLUMN staging.stg_operators.operator_id IS
  'PK. Format: OPR-001.';
...
-- Source: MinIO s3://raw/jeepney/stops/date=YYYY-MM-DD/stops.parquet
...
CREATE TABLE IF NOT EXISTS staging.stg_stops (
  ...
  CONSTRAINT chk_stops_type
    CHECK (stop_type IN (
      'terminal', 'market', 'school', 'hospital', 'mall', 'residential'))
);

COMMENT ON TABLE  staging.stg_stops IS
  'Silver: ~180 stops anchored on real Davao City landmarks. Grain: one row per stop.';
COMMENT ON COLUMN staging.stg_stops.stop_id IS
  'PK. Format: S001–S180.';
COMMENT ON COLUMN staging.stg_stops.latitude IS
  'GPS latitude. Valid range: Davao City geographic bounds (6.8–7.5).';
COMMENT ON COLUMN staging.stg_stops.longitude IS
  'GPS longitude. Valid range: Davao City geographic bounds (125.0–126.0).';
COMMENT ON COLUMN staging.stg_stops.has_shelter IS
  'TRUE if stop has a waiting shed. Approx 70% of urban stops = TRUE.';
COMMENT ON COLUMN staging.stg_stops.stop_type IS
  'Category of the landmark: terminal, market, school, hospital, mall, residential.';


-- =============================================================================
-- 4. staging.stg_vehicles
-- Grain : One row per jeepney unit
-- Source: MinIO s3://raw/jeepney/vehicles/date=YYYY-MM-DD/vehicles.parquet
...
COMMENT ON COLUMN staging.stg_vehicles.vehicle_id IS
  'PK. Format: VHC-001.';
...
COMMENT ON COLUMN staging.stg_vehicles.vehicle_type IS
  'traditional = old-style jeepney. modernized_PUV = Euro4/EV under LTFRB program.';


-- =============================================================================
-- 5. staging.stg_trips
-- Grain : One row per trip run (one vehicle, one direction, one day)
-- Source: MinIO s3://raw/jeepney/trips/date=YYYY-MM-DD/trips.parquet
...
COMMENT ON COLUMN staging.stg_trips.trip_id IS
  'PK. Format: TRP-00001.';
COMMENT ON COLUMN staging.stg_trips.load_factor IS
  'passengers_boarded / vehicle capacity. Values above 1.0 indicate overcrowding (allowed up to 1.5).';
COMMENT ON COLUMN staging.stg_trips.is_on_time IS
  'TRUE when delay_min <= 5. Enforced by chk_trips_on_time_logic constraint.';
COMMENT ON COLUMN staging.stg_trips.is_rainy_day IS
  'TRUE ~40% of days — based on Davao City average annual rainfall pattern.';
COMMENT ON COLUMN staging.stg_trips.delay_min IS
  'Actual minus scheduled travel time in minutes. Always 0 when early or on-time.';
COMMENT ON COLUMN staging.stg_trips.time_period IS
  'AM_peak = 6am–9am, midday = 9am–5pm, PM_peak = 5pm–8pm, off_peak = other hours.';


-- =============================================================================
-- 6. staging.stg_passenger_survey
-- Grain : One row per survey respondent
-- Source: MinIO s3://raw/jeepney/passengers/date=YYYY-MM-DD/passengers.parquet
-- Volume: 5,000 rows
...
COMMENT ON COLUMN staging.stg_passenger_survey.passenger_id IS
  'PK. Format: PAX-0001.';
COMMENT ON COLUMN staging.stg_passenger_survey.cluster_id IS
  'NULL at load time. Written by science/clustering.py after K-Means model runs. Values: 0–4.';
COMMENT ON COLUMN staging.stg_passenger_survey.cluster_label IS
  ...
  'Example: Underserved Riders, Student Commuters.';
COMMENT ON COLUMN staging.stg_passenger_survey.satisfaction_score IS
  '1=very poor, 2=poor, 3=fair, 4=good, 5=excellent.';
COMMENT ON COLUMN staging.stg_passenger_survey.avg_fare_paid_php IS
  'Average one-way fare paid. Must be > 0. No hard lower bound — pre-2026 rates may apply.';


-- =============================================================================
-- 7. staging.stg_ab_experiment
-- Grain : One row per passenger per test week in the A/B experiment
-- Source: MinIO s3://raw/jeepney/ab_experiment/date=YYYY-MM-DD/ab_experiment.parquet
-- Volume: ~8,000 rows (est. 1,000 Cluster 3 passengers × 8 weeks)
-- NOTE  : Only Cluster 3 (Underserved Riders) passengers are included here.
--         Statistical outputs (p_value, effect_size, CI) are NOT in this table —
--         they live only in marts.mart_ab_test_results after ab_testing.py runs.
-- =============================================================================
...
COMMENT ON TABLE  staging.stg_ab_experiment IS
  ...
  'Grain: one row per passenger per week. ~8,000 rows total (1,000 passengers × 8 weeks).';
COMMENT ON COLUMN staging.stg_ab_experiment.experiment_record_id IS
  'PK. Unique per passenger + week combination.';
COMMENT ON COLUMN staging.stg_ab_experiment."group" IS
  'Quoted because GROUP is a PostgreSQL reserved word. Values: control | treatment.';
COMMENT ON COLUMN staging.stg_ab_experiment.cluster_id IS
  'Always 3 (Underserved Riders). Enforced by chk_ab_cluster_only_3 constraint.';
COMMENT ON COLUMN staging.stg_ab_experiment.route_variant IS
  'A_existing_route = current path (1–2 transfers, ~55 min avg). '
  'B_express_direct = new direct route (0 transfers, ~35 min avg). '
  'Paired with "group" via chk_ab_group_variant_match: control→A_existing_route, treatment→B_express_direct.';


-- =============================================================================
-- END OF 06_tables_staging.sql
-- =============================================================================
