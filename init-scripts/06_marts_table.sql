...
-- ORDER   : Run AFTER 05_tables_staging.sql
--
-- All mart tables are populated by dbt at runtime — do not insert manually.
-- mart_commuter_clusters and mart_ab_test_results are shells until
-- science/clustering.py and science/ab_testing.py run first.
-- =============================================================================
...
-- Source : stg_trips JOIN stg_routes JOIN stg_vehicles (built by dbt)
...
COMMENT ON COLUMN marts.mart_district_ridership.avg_wait_time_min IS
  'From stg_passenger_survey. NULL for districts with no survey respondents.';
...
-- Volume : 5,000 rows
-- Serves : Superset Dashboard 2 + Streamlit cluster explorer
-- NOTE   : Shell only until science/clustering.py writes cluster_id and
--          cluster_label back to stg_passenger_survey, then dbt populates this.
-- =============================================================================
...
COMMENT ON TABLE  marts.mart_commuter_clusters IS
  ...
  'Grain: one row per passenger. Shell until science/clustering.py + dbt run. Built by dbt.';
...
-- NOTE   : p_value, is_significant, effect_size, CI columns are NULL until
--          science/ab_testing.py runs, then dbt builds this mart.
-- =============================================================================
