...
--
-- Statistical columns currently default to NULL in the dbt mart layer.
-- A/B statistical outputs are generated by science/ab_testing.py and
-- exported as Parquet artifacts for downstream analytics and reporting.
--
-- Future enhancement:
-- Persist statistical outputs into an intermediate science table
-- (e.g. intermediate.int_ab_test_statistics) and let dbt surface them here.
-- =============================================================================
...
{{
  config(
    ...
    post_hook    = "comment on table {{ this }} is 'Gold: A/B experiment mart for Cluster 3 underserved riders.'"
  )
}}
...
clusters as (
    ...
    where cluster_label = 'Underserved Riders'
)
