...
# Cluster → district bias
CLUSTER_DISTRICT_BIAS: dict[int, list[str]] = {
    0: ["Talomo", "Buhangin", "Agdao"],           # Students near university corridors
    1: ["Toril", "Calinan", "Tugbok"],             # Market workers from outskirts
    2: ["Poblacion", "Buhangin", "Talomo"],        # CBD workers inner city
    3: ["Paquibato", "Marilog", "Bunawan", "Calinan"],  # Underserved remote areas
    4: ["Talomo", "Buhangin", "Toril", "Agdao"],  # Spread — occasional riders
}

# Cluster → route bias (primary routes used)
CLUSTER_ROUTE_BIAS: dict[int, list[str]] = {
    0: ["R02", "R06", "R12", "R04"],   # Near schools, SM Lanang, City Hall
    1: ["R01", "R05", "R09"],           # Bankerohan-Toril, Calinan routes
    2: ["R02", "R06", "R11", "R07"],   # Ecoland–SM, CBD, Express
    3: ["R05", "R09", "R10", "R03"],   # Long routes, poor connectivity
    4: ["R01", "R02", "R04", "R08"],   # Various — occasional use
}
...
def cluster_params(cluster_id: int) -> dict:
    ...
    if cluster_id == 0:   # Student Commuters
        return dict(
            ...
            avg_fare          = round(float(np.clip(rng.normal(14.5, 1.0), 13.0, 18.0)), 2),
        ...
        return dict(
            ...
            avg_fare          = round(float(np.clip(rng.normal(16.0, 2.0), 13.0, 25.0)), 2),
            transfers         = int(np.clip(rng.integers(0, 2), 0, 3)),
        ...
        return dict(
            ...
            avg_fare          = round(float(np.clip(rng.normal(19.0, 3.0), 13.0, 35.0)), 2),
        ...
        return dict(
            ...
            avg_fare          = round(float(np.clip(rng.normal(22.0, 4.0), 13.0, 40.0)), 2),
        ...
        return dict(
            ...
            avg_fare          = round(float(np.clip(rng.normal(15.0, 2.0), 13.0, 25.0)), 2),
...
def generate_passengers() -> list[dict]:
    ...
    for cluster_id, count in CLUSTER_SIZES.items():
        ...
        for _ in range(count):
            ...
            origin_barangay = random.choice(DISTRICT_BARANGAYS[origin_district])
...
def main():
    ...
    print(f"[produce_passengers] Generating {total:,} passenger survey records")
    ...
    with httpx.Client(timeout=60) as client:
        for i in range(0, total, CHUNK_SIZE):
            ...
            print(f"[produce_passengers]   chunk {i//CHUNK_SIZE + 1}: {resp.json()}")
